...
async function seed() {
  ...
  await prisma.execution.deleteMany({ where: { tenant_id: DEMO_TENANT_ID } });
  await prisma.workflow.deleteMany({ where: { tenant_id: DEMO_TENANT_ID } });
  ...
  const step5 = await prisma.step.create({
    data: {
      workflow_id: workflow.id,
  ...
  const alertStep1 = await prisma.step.create({
    data: {
  ...
  const alertStep2 = await prisma.step.create({
    data: {
      workflow_id: workflow2.id,
